...
fun NotificationLogItemView(item: NotificationLogItem) {
    ...
    Column(modifier = Modifier.padding(vertical = 8.dp)) {
        ...
        Text("${item.street}, ${item.city}, ${item.state}, ${item.country}", fontWeight = FontWeight.Bold)
        Text("심각도: ${item.severity}")
        Text("좌표: ${item.location?.latitude}, ${item.location?.longitude}")
    }
}